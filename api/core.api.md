...
```ts
export type AnimationKind = "tween" | "spring" | "keyframes" | "bezier" | "catmull-rom" | "motion-path" | "arc" | "morph-path" | "sequence" | "parallel" | "stagger" | "loop" | "delay" | "reverse" | "map" | "raw";
```
...
### interface `CreateLayoutGroupOpts`
...
```ts
export type KeyframeStops = Record<string, readonly [
    unknown,
    unknown,
    ...unknown[]
]>;
...
### interface `MotionTransitionShape`
...
### interface `PlayViewTransitionOpts`
...
```ts
export type Point2List = readonly [
    Point2,
    Point2,
    ...Point2[]
];
...
### interface `ReorderControllerOpts`
...
export interface Timeline {
    ...
    at(position: TimelinePosition, def: AnimationDef<AnimationProps>, target: Target, opts?: Omit<TimelineAddOpts, "at">): Timeline;
...
```ts
export type TrackerBackend = StrategyBackend | "follow" | "scroll" | "scrub" | "ambient" | "view-transition" | "timeline";
```
...
```ts
export type TransformInputRange = readonly [
    number,
    number,
    ...number[]
];
...
```ts
export type TransformOutputRange<T> = readonly [
    T,
    T,
    ...T[]
];
...
```ts
export type TweenProps = Record<string, readonly [
    unknown,
    unknown
]>;
```
...
export type TweenValue<P extends TweenProps> = {
    [K in keyof P]: P[K] extends readonly [
        infer A,
        infer B
    ] ? Widen<A | B> : never;
};
...
```ts
export declare function arc(cx: number, cy: number, radius: number, fromAngle: number, toAngle: number, opts?: ArcOpts): AnimationDef<BezierPathValue>;
```
...
```ts
export declare function bezierPath(points: Point2List, opts?: BezierPathOpts): AnimationDef<BezierPathValue>;
```

### function `bezierPathLength`

```ts
export declare function bezierPathLength(points: Point2List, samplesPerSegment?: number): number;
```

### function `bindMotionValueToCss`

```ts
export declare function bindMotionValueToCss<T>(mv: MotionValue<T>, el: {
    style: {
        setProperty(name: string, value: string): void;
    };
}, cssVarName: string, format?: (value: T) => string): Unsubscribe;
```
...
```ts
export declare function catmullRom(waypoints: Point2List, opts?: CatmullRomOpts): AnimationDef<BezierPathValue>;
```

### function `catmullRomToCubicPoints`

```ts
export declare function catmullRomToCubicPoints(waypoints: Point2List, tension: number, closed: boolean): Point2List;
```
...
```ts
export declare function combine<S extends readonly MotionValue<unknown>[], T>(sources: S, fn: (...values: SourceValues<S>) => T): CombinedMotionValue<T>;
```
...
```ts
export declare function createLayoutGroup(opts?: CreateLayoutGroupOpts): LayoutGroup;
```

### function `createReorderController`

```ts
export declare function createReorderController<T>(opts: ReorderControllerOpts<T>): ReorderController<T>;
```
...
```ts
export declare function deCasteljau(points: Point2List, t: number): Point2;
```
...
```ts
declare const defaultLayoutGroup: LayoutGroup
```
...
### function `easingLabel`

```ts
export declare function easingLabel(fn: EasingFn | undefined): string | undefined;
```
...
```ts
export declare function follow(targets: readonly FollowTarget[], opts?: FollowOpts): FollowHandle;
```
...
```ts
export declare function inView(target: Element, enter: InViewEnter, opts?: InViewOpts): InViewStop;
```
...
```ts
export declare function interpolate(from: number, to: NoInfer<number>): (progress: number) => number;
export declare function interpolate(from: string, to: NoInfer<string>): (progress: number) => string;
export declare function interpolate(from: readonly number[], to: NoInfer<readonly number[]>): (progress: number) => number[];
export declare function interpolate<T>(from: T, to: NoInfer<T>): (progress: number) => T;
```
...
```ts
export declare function interpolateNumbers(from: readonly number[], to: readonly number[]): (progress: number) => number[];
```
...
```ts
export declare function jitter<T>(def: AnimationDef<T>, opts?: JitterOpts): AnimationDef<T>;
```

### function `keyframes`

```ts
export declare function keyframes<const P extends KeyframeStops>(stops: P, opts?: KeyframesOpts): AnimationDef<KeyframesValue<P>>;
```
...
```ts
export declare function map<A, B>(anim: AnimationDef<A>, fn: (a: NoInfer<A>) => B): AnimationDef<B>;
```

### function `mat4`

```ts
export declare function mat4(loc: UniformLocation, transpose?: boolean): UniformBinding<Mat4>;
```

### function `morphPath`

```ts
export declare function morphPath(from: string, to: string, opts?: MorphPathOpts): AnimationDef<{
    d: string;
}>;
```
...
```ts
export declare function motionPath(d: string, opts?: BezierPathOpts): AnimationDef<BezierPathValue>;
```
...
```ts
declare const normalizedVelocity: (v: number) => NormalizedVelocity
```

### function `omitUndefined`

```ts
export declare function omitUndefined<T extends object>(input: T): {
...
```ts
export declare function playPan(opts: PanOpts): PanHandle;
```

### function `playPinch`

```ts
export declare function playPinch(opts: PinchOpts): PinchHandle;
```

### function `playPress`

```ts
export declare function playPress(opts: PressOpts): PressHandle;
```
...
```ts
export declare function playTap(opts: TapOpts): TapHandle;
```
...
```ts
export declare function playValues<V>(def: AnimationDef<V>, onValue: ValuesCommit<NoInfer<V>>, opts?: ValuesOpts): ValuesHandle;
```

### function `playViewTransition`

```ts
export declare function playViewTransition(mutate: () => void | Promise<void>, opts?: PlayViewTransitionOpts): Controls;
```
...
```ts
export declare function resolveTransition(t: MotionTransitionShape | undefined): ResolvedTransition;
```
...
```ts
export declare function sampleBezierPath(points: Point2List, samples: number, samplesPerSegment?: number): Point2[];
```
...
```ts
export declare function scrub(def: AnimationDef<AnimationProps>, targets: readonly StrategyTarget[], opts?: ScrubOpts): ScrubHandle;
```
...
```ts
export declare function spring<const P extends TweenProps>(props: P, opts?: SpringOpts): AnimationDef<TweenValue<P>>;
```
...
### function `svgPathLength`

```ts
export declare function svgPathLength(d: string, samplesPerSegment?: number): number;
```

### function `svgPathToCubicPoints`

```ts
export declare function svgPathToCubicPoints(d: string): Point2List;
```

### function `time`

```ts
export declare function time(): TimeMotionValue;
```
...
### function `trackAmbient`

```ts
export declare function trackAmbient(handle: AmbientHandle, backend: TrackerBackend, targets?: readonly StrategyTarget[], meta?: TrackerMeta): number;
```

### function `trackAnimation`

```ts
export declare function trackAnimation(controls: Controls, targets: readonly StrategyTarget[], backend?: TrackerBackend, meta?: TrackerMeta): number;
```

### function `trackNamed`

```ts
export declare function trackNamed(name: string, backend?: TrackerBackend): () => void;
```

### function `transform`

```ts
export declare function transform<T>(inputRange: TransformInputRange, outputRange: TransformOutputRange<T>, opts?: TransformOpts): (input: number) => T;
```

### function `tween`

```ts
export declare function tween<const P extends TweenProps>(props: P, opts?: TweenOpts): AnimationDef<TweenValue<P>>;
```

### function `untrackAmbient`

```ts
export declare function untrackAmbient(id: number): void;
```

### function `vec2`

```ts
export declare function vec2(loc: UniformLocation): UniformBinding<Vec2>;
```

### function `vec3`

```ts
export declare function vec3(loc: UniformLocation): UniformBinding<Vec3>;
```

### function `vec4`

```ts
export declare function vec4(loc: UniformLocation): UniformBinding<Vec4>;
```

### function `velocity`

```ts
export declare function velocity(source: MotionValue<number>): VelocityMotionValue;
```

### const `velocityFromSpan`

```ts
declare const velocityFromSpan: (realVelocity: number, span: number) => NormalizedVelocity
```
